...
from genetic_data import path_wujico,df_recipes_ingredients,columns_nutri,profil_min
from genetic_classes import list_family,jour,repas,ptitdej,dejdin,col,ingredient
from genetic_profils import det_grouped_profils

#%%
'''
All function for genetic algo
'''
...
#%%
#def visu_recettes(population,iteration,group_number):
#    
#    # ax.invert_yaxis()
#    # ax.xaxis.set_visible(False)
#    
#    fig, ax = plt.subplots(figsize=(9.2, 5))
##    cmap = plt.get_cmap('gist_rainbow',df_recipes_ingredients.shape[0])
#    
#    for ind,individu in enumerate(population):
#        widths=[]
#        colors=[]
#        for i in range(4):
#            for j in range(len(individu.repas[i].ingrs)):
#                widths.append(individu.repas[i].ingrs[j].weight)
#                colors.append(df_ingredients[df_ingredients['alim_code']==individu.repas[i].ingrs[j].alim_code]['colormap'].to_list()[0])
##                colors.append(cmap(df_recipes_ingredients.reset_index().
##                                   index[df_recipes_ingredients['alim_code']==individu.repas[i].ingrs[j].alim_code]
##                                   .to_list()[0]/df_recipes_ingredients.shape[0]))
#            #to insert a white pad between dishes
#            widths.append(100)
#            colors.append((0,0,0,0))
#
#        widths_cum = np.cumsum(widths)
#        widths_cum=np.insert(widths_cum,0,0)
#        
#        for i in range(len(widths)):
#            ax.barh(ind, widths[i], left=widths_cum[i], height=0.5,color=colors[i])
#    plt.show()
#    fig.tight_layout()
#    fig.savefig('log_GA/'+d+'/plots/visu_recettes/visu_recettes_'+dt+'_gn_'+str(group_number)+'_pop_'+str(population_size)+'_pc_'+str(proba_crossover)+'_pdp_'+str(proba_double_pivot)+'_pm_'+
#                  str(proba_mutate)+'_cn_'+str(coeff_nutri)+'_ct_'+str(coeff_taste)+'_cm_'+str(coeff_mini)+'_cnov_'+str(coeff_novelty)+'_ni_'+str(nb_iteration)+'_iter_'+str(iteration)+"_"+tagfile+".eps",format='eps')
#    plt.close(fig)
...
#def print_recipes(population,probassortedindices,exp_combined,exp_nutri,exp_taste,indsbest,f,group_number):
#    print("------------------BEST MOST DIFFERENT RECIPES--------------------------------------------",file=f)
#    recip=["FIRST BEST","SECOND BEST","THIRD BEST"]
#    for i,ind in enumerate(indsbest):
#        print("-------------------------"+recip[i]+" RECIPES---------------------------------------------------",file=f)
...
#        print ("score_nutri =", population[ind].getScoreNutri(group_number),file=f)
#        print ("score_taste =", population[ind].getScoreTaste(),file=f)
#        print ("exp_combined =", exp_combined[ind],file=f)
#        print ("exp_nutri =", exp_nutri[ind],file=f)
#        print ("exp_taste =", exp_taste[ind],file=f)
#        print ("PRAL = ",np.mean([population[ind].repas[i].getPRAL() for i in range(4)]),file=f)
#        print ("SAIN = ",np.mean([population[ind].repas[i].getSAIN() for i in range(4)]),file=f)
#        print ("LIM = ",np.mean([population[ind].repas[i].getLIM() for i in range(4)]),file=f)
#        print ("FF = ",np.mean([population[ind].repas[i].getFF() for i in range(4)]),file=f)
#        print ("NutriScore = ",np.mean([population[ind].repas[i].getNutriScore() for i in range(4)]),file=f)
#        # print best recipes
#        print("+++++++++++++++++++++++++++++++++++++++++++++",file=f)
#        rep=["Petit Dejeuner:","Dejeuner:","Collation:","Diner:"]
#        for j in range(4):
#            print(rep[j],file=f)
#            for ing in population[ind].repas[j].ingrs:    
...
#            print("+++++++++++++++++++++++++++++++++++++++++++++",file=f)
#        #print nutrition 
#        df_nutri=pd.DataFrame([population[ind].repas[k].getNutriValueNorm(group_number) for k in range(4)])
#        df_nutri=df_nutri.transpose()
#        df_nutri.columns=rep
#        df_nutri['global']=population[ind].getNutriNormGlobal(group_number)
#        print(df_nutri,file=f)
#        #save nutrition to excel file
#        df_nutri.to_excel('log_GA/'+d+'/df_best_nutri/df_'+recip[i]+'_nutri_'+dt+'_profil_'+str(group_number)+'_rec_'+str(i)+'_pop_'+str(population_size)+'_pc_'+str(proba_crossover)+'_pm_'+
#          str(proba_mutate)+'_cn_'+str(coeff_nutri)+'_ni_'+str(nb_iteration)+'_iter_'+str(iteration)+'_gn_'+str(group_number)+'_'+tagfile+'.xlsx')
#        plot_nutrition(df_nutri)
#%%
        
def worker(iteration,group_number,pop_ingrs,profils_common_ingredients,
           all_scores_nutri,all_scores_taste):
     #fixe seed pour reproducibilité
#    random.seed(123)
#    np.random.seed(123)
    ...
    start_it = time.time()
#        probas_nutri=calcul_proba_nutri(population, group_number)
#        probas_taste=calcul_proba_taste(population)
...
#    random.seed(123)
#    np.random.seed(123)
    nb_profil=9 #np.random.randint(2,20)
...
#    archive=[]
#    for day in archive_recettes:
#        archive.append([set([df_recipes_ingredients[df_recipes_ingredients['alim_code']==j.alim_code]['keyword'].to_list()[0] for j in day.repas[i].ingrs]) for i in range(4)])
    ...
    nb_iteration = 200
    record_it=100
    #coefficient which set the importance of objective between nutrition and taste
    coeff_nutri=2
    coeff_taste=1.5
    coeff_mini=1
    coeff_novelty=1
    ...
    #saving prog genetic.py to be able to not loose track of how we obtained results
...
#        population = pickle.load(f)
    ...
    for iteration in range(nb_iteration+1):
        ...
        #calculation of profils_common_ingredients
#        for group_number in range(len(profil_grouped_names)):
#            print("pop_ingrs,gn= ",group_number,":")
#            print(pop_ingrs[group_number])
        profils_common_ingredients=pop_ingrs[0].intersection(*pop_ingrs[1:])
#        print("profils_common_ingredients:")
#        print(profils_common_ingredients)
...
#    ps.print_stats()
#    with open('log_GA/profiling.txt','w+') as f:
#        f.write(s.getvalue())
...
#        cmap = sns.diverging_palette(10, 150, n=10, as_cmap=True)
#        # Draw the heatmap with the mask and correct aspect ratio
#        sns.heatmap(df_formulation1_profil_percent,vmin=0,vmax=1.,square=True,cmap=cmap, annot=True,
#                    fmt=".0f", linewidths=.5, cbar_kws={"shrink": .5})   
#        plt.title("Formulation 1, ratio of daily intake with respect to recommended daily intake",fontsize=16)
#        plt.tight_layout()
#        f.savefig("df_GA_profil_percent_corrected.png",dpi=150)
